class MypagesController < ApplicationController
  ...
  def index
    ...
    @books_now = @user.books.where(status: 0).order(id: 'desc')

    # ユーザーに紐づいている書籍の内、ステータスが「今読んでいる」のものを取得
    @books_reading = @user.books.where(status: 1).order(id: 'desc')

    # ユーザーに紐づいている書籍の内、ステータスが「読み終わった」のものを取得
    @books_finished = @user.books.where(status: 2).order(id: 'desc')
